          ...
          <div className="card flex-1 p-6">
            ...
            <ul className="mt-5 space-y-4">
              {recentEvents.length === 0 && (
                <li className="text-sm text-slate-400">Nenhuma atividade ainda.</li>
              )}
              {recentEvents.map((ev) => (
                <li key={ev.id}>
                  <Link
                    href={`/leads/${ev.leadId}`}
                    className="block rounded-lg transition-colors hover:bg-slate-50 -mx-2 px-2 py-1"
                  >
                    <div className="flex flex-wrap items-center gap-1.5 text-xs">
                      <span className="font-medium text-slate-900">{ev.leadName}</span>
                    ...
                    <p className="mt-1 truncate text-sm text-slate-600">{ev.content}</p>
                  </Link>
                </li>
              ))}
            </ul>
